...
impl<H, L> Clocks<H, L, LfOscStopped> {
    ...
    pub fn set_lfclk_src_synth(
        self,
    ) -> Clocks<H, LfOscSynthesized, LfOscStopped> {
        self.periph.lfclksrc.write(|w| {
            w.src().synth().bypass().disabled().external().disabled()
        });
        Clocks {
            hfclk: self.hfclk,
    ...
    pub fn set_lfclk_src_external(
        ...
        self.periph.lfclksrc.write(move |w| {
            w.src().xtal().bypass().bit(byp).external().bit(ext)
        });
        Clocks {
            hfclk: self.hfclk,
